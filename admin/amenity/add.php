...
<body>
  ...
  <div class="dashboard-layout">
    ...
    <main class="dashboard-main">
      <header class="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 class="mb-1">Add New Amenity</h2>
          <p class="text-muted">Create a new amenity with an icon</p>
        </div>
      ...
      <div class="card border-0 shadow-sm">
        <div class="card-body p-4">
          <div class="mb-4">
            ...
            <input type="text" class="form-control" id="name" placeholder="e.g. Free Wi-Fi, Private Pool">
          </div>
          ...
          <div class="mb-4">
            ...
            <div class="icon-grid" id="iconGrid">
              <?php foreach ($commonIcons as $icon): ?>
                <div class="icon-item" onclick="selectIcon('<?php echo $icon; ?>', this)" data-icon="<?php echo $icon; ?>">
                  <i class="bi <?php echo $icon; ?>"></i>
                </div>
              <?php endforeach; ?>
            </div>
            <input type="hidden" id="icon_cls" value="">
          </div>

          <div class="selected-icon-preview mb-4 d-none" id="previewContainer">
            <p class="fw-semibold mb-2">Selected Preview:</p>
            <div class="d-flex align-items-center gap-3 p-3 bg-light rounded">
              <i id="previewIcon" class="bi fs-2 text-primary"></i>
              <span id="previewText" class="text-muted small"></span>
            </div>
          </div>

          <div class="text-end">
            <button onclick="addAmenity();" class="btn btn-primary px-5">
              <i class="fa-solid fa-save me-2"></i>Save Amenity
            </button>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    function addAmenity() {
      ...
      PostRequest('/lib/add-amenity-process.php', form, function(response, error) {
        if (error) {
